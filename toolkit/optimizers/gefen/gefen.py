...
from .partitioning import divisors, find_period_by_block_variance
from . import quantization as quantization_module
...
def automatic_vmean_update(
    ...
    *,
    ...
    if use_fused:
        ...
        from .kernels.automatic_vmean import automatic_vmean_update_cuda
...
def gefen_automatic_fused_update(
    *,
    ...
    from .kernels.automatic_gefen_fused import automatic_gefen_fused_update_cuda
...
def gefen_automatic_fused_update_from_vmean(
    *,
    ...
    from .kernels.automatic_gefen_fused import (
...
def gefen_automatic_period1_update(
    *,
    ...
    from .kernels.automatic_gefen_fused import automatic_gefen_period1_update_cuda
    ...
    try:
        for _, flat, period, _ in grad_periods:
            if use_fused_histogram and flat.device.type == "cuda":
                from .kernels.exact_histogram_fused import (
...
class Gefen(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        *,
        ...
        self._exit_after_grad_sq_save = False
    ...
    def _automatic_unview_update(
        self, update_view: torch.Tensor, grad: torch.Tensor, period: int
    ) -> torch.Tensor:

        return update_view.view(grad.size())
    ...
    def _maybe_save_grad_sq_and_exit(
        self, param_name: str, grad: torch.Tensor, automatic_period: int
    ) -> None:
        parameter_name_to_save = globals().get("GRAD_SQ_PARAMETER_NAME_TO_SAVE")
        if parameter_name_to_save is None:
            return
        elif not isinstance(parameter_name_to_save, str):
            raise TypeError(
                "GRAD_SQ_PARAMETER_NAME_TO_SAVE must be a str, got {}".format(
                    type(parameter_name_to_save).__name__
                )
            )
        elif parameter_name_to_save.lower() != param_name:
            return

        output_path = Path.cwd() / "{}_grad_sq.pt".format(_safe_filename(param_name))
        grad_sq_values = grad.detach().float().square().reshape(-1).cpu()
        torch.save(grad_sq_values, output_path)
        print(
            "Gefen automatic period for {} is {}".format(param_name, automatic_period),
            flush=True,
        )
        print(
            "Saved Gefen grad^2 tensor to {}".format(output_path.resolve()), flush=True
        )

        self._exit_after_grad_sq_save = True
    ...
    def _print_period_results_enabled(self) -> bool:
        print_period_results = globals().get("PPR", False)
        if not isinstance(print_period_results, bool):
            raise TypeError(
                "PPR must be a bool, got {}".format(type(print_period_results).__name__)
            )
        return print_period_results
    ...
    def _predict_period_from_grad_sq(
        ...
        print_period_results = self._print_period_results_enabled()
        if self._cap_period_by_shape_enabled():
            parameter_shape = self._parameter_shape_for_period_cap(param)
        ...
        if backend == "cuda_kernel":
            ...
            try:
                return find_period_by_block_variance(
                    grad_flat,
                    print_results=print_period_results,
    ...
    def _iter_gefen_grad_periods(self, reuse_existing_periods: bool = False):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if reuse_existing_periods or "step" in self.state.get(p, {}):
                    state = self.state[p]
    ...
    def _expected_magnitude_rows(self, state, grad_view: torch.Tensor) -> int:
        if grad_view.shape[1] == 1:
            period_one_divisor = self._period_one_divisor_from_state(
                state, grad_view.numel()
            )
            return grad_view.numel() // period_one_divisor
        elif grad_view.shape[1] > 1:
            return grad_view.shape[0]
        raise ValueError(
            "Expected grad_view second dimension to be positive, got {}".format(
                grad_view.shape[1]
            )
    ...
    def _repair_gefen_state_for_grad_view(
        ...
        # We repair misaligned/corrupted optimizer state after resume so fused
        # kernels do not see float32 m_codebook or mismatched block counts.
        expected_shape = tuple(grad_view.shape)
        device = grad_view.device
        state_dtype = gefen_state_dtype_from_grad(grad_view)
        magnitude_rows = self._expected_magnitude_rows(state, grad_view)
        magnitude_shape = (magnitude_rows, 1)
        vmean_shape = (grad_view.shape[0], 1)
        ...
        if (
            ...
            or m_magnitude.dtype != state_dtype
            or tuple(m_magnitude.shape) != magnitude_shape
        ):
        ...
        if (
            ...
            or vmean.dtype != state_dtype
            or tuple(vmean.shape) != vmean_shape
        ):
    ...
    def _step_automatic(
        ...
        if "step" not in state:
            ...
            self._maybe_save_grad_sq_and_exit(param_name, grad, automatic_period)
            state["step"] = 0
        ...
        if use_fused_gefen_step:
            ...
            update = self._automatic_unview_update(
                shared_m * stepsize, grad, automatic_period
            )
            update.mul_(lr)
            local_p.add_(-update)
    ...
    def step(self, closure=None):
        ...
        if self._exit_after_grad_sq_save:
            raise SystemExit(0)
        return loss
